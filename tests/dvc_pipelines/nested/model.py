...
def find_hyperparameters() -> None:
    ...
    with Path("tests/dvc_pipelines/nested/files/hyperparameters.json").open("w") as file:
...
def train_model(kind: str | None) -> None:
    ...
    with Path("tests/dvc_pipelines/nested/files/hyperparameters.json").open() as file:
    ...
    with Path("tests/dvc_pipelines/nested/files/blue.json").open() as file:
