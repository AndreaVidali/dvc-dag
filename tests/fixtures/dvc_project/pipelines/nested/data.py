...
def import_data(color: str) -> None:
    """Import data."""
    with Path(f"pipelines/nested/data/raw_{color}.json").open() as file:
        data = json.load(file)

    imported_data = {key.replace("raw_", ""): value for key, value in data.items()}

    with Path(f"pipelines/nested/data/{color}.json").open("w") as file:
...
def do_unrelated_stuff() -> None:
    """Do unrelated stuff."""
    with Path("pipelines/nested/data/unrelated.json").open("w") as file:
